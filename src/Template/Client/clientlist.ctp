...
<div class="panel-body">	
		...
		<div id="example_wrapper" class="dataTables_wrapper">
			<table id="plist" class="table table-striped" cellspacing="0" width="100%">
				...
				<tbody>
					...
					<tr class="row_<?php echo $client_info['user_id']; ?>">
	...
	?>
						...
						<a href="<?php echo $this->Url->build(array('controller'=>'Client','action'=>'updateclient',$client_info['user_id']))?>" class="btn btn-info">Edit</a>&nbsp;<a url ="<?php echo $this->Url->build(array('controller'=>'Client','action'=>'delete',$client_info['user_id']))?>" class="btn btn-danger <?php if($data == 0){ echo 'sa-warning'; }else{ echo 'notdelete'; } ?>">Delete</a>&nbsp;
						<a url="<?php echo $this->Url->build(array('controller'=>'Client','action'=>'archiveclient',$client_info['user_id']))?>" clientid="<?php echo $client_info['user_id']; ?>" class="btn btn-info archive_value">Archive</a>
	...
	$('body').on('click','.archive_value',function(){
		...
		swal({   
			...
			if (isConfirm) {
				...
				$.ajax({
				   type:'POST',
				   url:'<?php echo $this->Url->build(['controller'=>'ajax','action'=>'archiveclient']);?>',
				   data:{clientid:clientid},
				   success:function(data){
						$('.row_'+clientid).fadeOut();	
