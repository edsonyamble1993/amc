...
$invoice_no=(isset($update_row)) ? $update_row['invoice_no']: $invoice_idf ;
...
$reference=(isset($update_row))?$update_row['reference_no']:'';
$address=(isset($update_row))?$update_row['address']:'';
$subject=(isset($update_row))?$update_row['remark']:'';
...
array('controller'=>'Invoice','action' => 'viewinvoice'),array('escape' => false));
					  ...
					  <li class="<?php echo $action=='updateinvoice'?'active':''?>">
<?php  echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-plus-circle fa-lg')) .__('Update Invoice'),array('controller'=>'Invoice','action' => 'updateinvoice'),array('escape' => false));
						?>
		...
		dateFormat: "yy-mm-dd",
          ...
          <?php echo $this->Form->Create('form1',['id'=>'client_form','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data'],['url'=>['action'=>'']]);?>
				...
				<div class="form-group">
						...
						<div class="col-sm-4">
							<?php echo $this->Form->input('',array('name'=>'reference_no','value'=>$reference,'class'=>'form-control validate[required]','placeholder'=>__('')));?>
						</div>
		...
		<option value="<?php echo $client_row['user_id'];?>" ><?php echo $client_row['first_name'].' '.$client_row['last_name']; ?></option>
		  ...
		  <button type="button" id="add_newrow" class="btn btn-default" style="margin:5px 0px;">Add New </button>
				...
				<?php $row_id = 0;?>
                  <?php 
                    if(isset($ih_histroy)){
                        foreach($ih_histroy as $ih_row){

                
                ?>
						<tr id="row_id_<?php echo $ih_row['invoice_h_id'];?>">
							<td>
			...
			<option value="<?php echo $product_info['product_id']?>" <?php 
					if($product_info['product_id'] == $ih_row['item_name']){
            ...
            ?> ><?php echo $product_info['product_code'].' '.$product_info['item_name']; ?></option>
							...
							<td>
								<input type="text" name="product[quantity][]" class="quantity form-control" data-id ="<?php echo $row_id;?>" value="<?php echo $ih_row['qty'];?>" id="quantity_<?php echo $row_id;?>">
							</td>
							<td>
								<input type="text" name="product[price][]" class="product form-control" data-id ="<?php echo $row_id;?>" value="<?php echo $ih_row['price'];?>" id="price_<?php echo $row_id;?>" readonly='true'>
							</td>
							<td>
								<input type="text" name="product[amount][]" class="product form-control" data-id ="<?php echo $row_id;?>" value="<?php echo $ih_row['net_amount'];?>" id="amount_<?php echo $row_id;?>" readonly='true'>
							</td>
