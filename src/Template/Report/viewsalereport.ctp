...
array('controller'=>'report','action' => 'viewservicesreports'),array('escape' => false));
				...
				<form method="post" accept-charset="utf-8" id="client_form" class="form_horizontal formsize" enctype="multipart/form-data" action="<?php echo $this->Url->build(array('controller'=>'report','action'=>'viewsalereport'))?>">
					<div class="header">
						<h3><?php echo __('Sales Reports'); ?></h3>
					</div><br/><br/>
         ...
         <option value="<?php echo $client_row['user_id']; ?>" <?php 
				...
				<?php
							...
							$chart_array[] = array(date('M-Y',strtotime($result['date'])),(int)$result['count']);
							
							$chart_sum[] = $result['count'];
    						}
							echo "<h6 style='font-weight:bold;font-size:14px;padding-left:30px;text-transform:uppercase;'> <u style='padding-bottom:2px;'>".$start_date."</u> Until <u>".$end_date."</u> : Total ".array_sum($chart_sum)." Sales</h6>";
