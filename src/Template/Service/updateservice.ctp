...
array('controller'=>'Service','action' => 'viewservice'),array('escape' => false));
  ...
  <?php  echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-plus-circle fa-lg')) .__('Update Service'),array(),array('escape' => false));
						?>
					  </li>
	...
	jQuery('#date').datepicker({
		dateFormat: "yy-mm-dd",
          ...
          <?php echo $this->Form->Create('form1',['id'=>'client_form','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data'],['url'=>['action'=>'']]);?>
				...
				<div class="form-group">
						...
						<div class="col-sm-4">
							<?php echo $this->Form->input('',array('name'=>'service_code','value'=>$update_row['service_code'],'class'=>'form-control validate[required]','readonly'));?>
						</div>
						...
						<div class="col-sm-4">
							<select name="assign_to" class="form-control" required="required">
                                ...
                                <?php
                                if($employee_info){
                                    foreach($employee_info as $employee_name){ ?>
								<option value="<?php echo $employee_name['user_id']; ?>" <?php 
                                            if(isset($update_row)){
                                                
                                                if($update_row['assign_to'] == $employee_name['user_id']){
                                                    echo 'selected="selected"';
                                                }
                                                
                                            }
                                        
                                        ?>><?php echo $employee_name['first_name'].' '.$employee_name['last_name']; ?></option>
								<?php
                                            }
                                }
                                ?>
				...
				<div class="form-group">
							...
							<div class="col-sm-4">
								<?php //echo $this->Form->input('',array('name'=>'date','value'=>date($this->Amc->getDateFormat(),strtotime($update_row['date'])),'id'=>'date','class'=>'form-control','placeholder'=>__('Date'),'required'=>'required'));?>
								<input type="date" name="date" class="form-control" placeholder="Date" value="<?php echo date("Y-m-d",strtotime($update_row['date'])); ?>" id="">
							</div>
							...
							<option value="1" <?php if($update_row['status'] == 1){ echo 'selected'; } ?>>Open</option>
						<option value="2" <?php if($update_row['status'] == 2){ echo 'selected'; } ?>>Progress</option>
						<option value="0" <?php if($update_row['status'] == 0){ echo 'selected'; } ?>>Closed</option>
		...
		<option value="<?php echo $client_row['user_id'];?>" <?php 
                ...
                ?>><?php echo $client_row['first_name'].' '.$client_row['last_name']; ?></option>
                    ...
                    <div class="col-sm-2 label_right"><?php echo $this->Form->label(__('Service Charge'));?></div>
                    
							<div class="col-sm-2" >
                                <select class="form-control" name="charge" id="service_charge">
                                    <option value="nocharge" <?php if($update_row['charge'] == 'nocharge'){echo 'selected="selected"';}?> >No Charge</option>
                                    <option value="charge" <?php if($update_row['charge'] == 'charge'){echo 'selected="selected"';}?>>Charge</option>
                                </select>
							</div>
                    
                    <div class="col-sm-2">
                               <input type="text" placeholder="charge" value="<?php echo $update_row['charge_amount'];?>" class="form-control" name="charge_amount" id="charge_box" style="display:none">
							</div>
...
$(document).ready(function(){
    ...
    if($("select[name=charge]").val() == 'charge'){
         $("#charge_box").show(200);
    }
    ...
    <div class="form-group">
						...
						<div class="col-sm-4">
							<?php echo $this->Form->textarea('',array('name'=>'service_details','value'=>$update_row['service_details'],'class'=>'form-control validate[required]','required'=>'required'));?>
						</div>
     <div class="col-sm-2 label_right"><?php echo $this->Form->label(__('Remark:'));?></div>
                    
							<div class="col-sm-4">
								<?php echo $this->Form->textarea('',array('name'=>'remark','value'=>$update_row['remark'],'class'=>'form-control validate[required]'));?>
							</div>
				 ...
				 <div class="header">
          		<h3><?php echo __('Service Details'); ?></h3>
          </div>
...
<tbody>
				<?php $row_id = 0;?>
                    <?php 
                        if(isset($svh_histroy)){
                            
                            foreach($svh_histroy as $history){
			...
			<option value="<?php echo $product_info['product_id']?>" <?php 
                            if(isset($history)){
                                if($history['item_name'] == $product_info['product_id']){
                                    echo 'selected="selected"';
                                }
                    ...
                    ?>><?php echo $product_info['product_code'].' '.$product_info['item_name']; ?></option>
							...
							<td>
								<input type="text" name="product[note][]" class="form-control" data-id ="<?php echo $row_id;?>" value="<?php echo $history['note']; ?>" id="note_<?php echo $row_id;?>">
							</td>
